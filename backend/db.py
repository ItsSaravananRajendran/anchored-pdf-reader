"""SQLite schema and helpers for the PDF reader."""
import sqlite3
import time
import json
...
def upsert_pdf(hash_: str, url: str, bytes_: int, path: str, page_count: Optional[int], title: Optional[str]) -> None:
    with conn() as c:
        c.execute(
...
def get_pdf(hash_: str) -> Optional[sqlite3.Row]:
    with conn() as c:
...
def delete_anchor_for_message(message_id: str) -> int:
    ...
    Identifies the pair by shared session_id + (page, rect, rotation). Returns rows deleted.
    """
    with conn() as c:
        row = c.execute(
        ...
        if row is None:
            return 0
        # The anchor pair shares the same anchor metadata and is within a few seconds
        # (created close together). To avoid accidentally deleting unrelated messages,
        # we delete ONLY:
        #   - messages with the same session_id, same anchor metadata, and role != this role
        #   - and the message itself
        # This is the safe interpretation: a user clicks delete on a chip and the entire
        # Q&A pair (user + assistant) is removed.
        if row["anchor_page"] is None:
            # Not an anchored message — just delete it
...
def update_message_text(msg_id: str, text: str, status: str = "complete", error_kind: Optional[str] = None) -> None:
    with conn() as c:
        c.execute(
